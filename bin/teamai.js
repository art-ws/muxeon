...
//
// TEAMAI is a Bun application, not a Node one: the four HTTP surfaces are
...
//
// `bunx teamai` skips this file entirely and runs dist/index.js directly.
...
if (!existsSync(entry)) {
  process.stderr.write(
    `teamai: the bundle is missing (${entry}).
...
// TEAMAI_BUN lets an operator pin a specific runtime (a pinned version, a
// non-PATH install); otherwise the one on PATH.
const bun = process.env.TEAMAI_BUN ?? "bun";
...
child.on("error", (error) => {
  if (error.code === "ENOENT") {
    process.stderr.write(
      `teamai: bun not found (tried "${bun}").

TEAMAI runs on Bun. Install it, then re-run:
  curl -fsSL https://bun.sh/install | bash

Already installed elsewhere? Point TEAMAI_BUN at it:
  TEAMAI_BUN=/path/to/bun npx teamai
...
`,
  ...
  process.stderr.write(`teamai: failed to start bun: ${error.message}\n`);
  process.exit(1);
});
