...
// the served shell as `<meta name="muxeon-name">` (and the matching page title).
...
export function instanceName(): string {
  ...
  const meta = document.querySelector('meta[name="muxeon-name"]');
  return meta?.getAttribute("content")?.trim() ?? "";
}

/**
 * The panel's identity mode (§17.2, FR-127): the connector injects
 * `<meta name="muxeon-auth" content="users">` into the served shell when the
...
export function authMode(): "users" | "legacy" {
  ...
  const meta = document.querySelector('meta[name="muxeon-auth"]');
